...
export LLM_JUDGE_MODEL=same-as-agent
...
grep -q '"llm_judge_model": "openai/gpt-4o-mini"' "${meta}" || fail "judge model missing in metadata"
...
assert data["run"]["llm_judge_model"] == "openai/gpt-4o-mini", data
...
grep -q 'Judge model: `openai/gpt-4o-mini`' "${GITHUB_STEP_SUMMARY}" || fail "summary missing judge"
grep -q 'Passed: 1, Failed: 1' "${GITHUB_STEP_SUMMARY}" || fail "summary missing counts"
pass "GITHUB_STEP_SUMMARY"

# Explicit judge override
export LLM_JUDGE_MODEL=google/gemini-2.5-flash
: >"${GITHUB_OUTPUT}"
: >"${GITHUB_STEP_SUMMARY}"
python3 "${SCRIPT}"
grep -q '"llm_judge_model": "google/gemini-2.5-flash"' "${meta}" || fail "judge override missing"
pass "judge override"
